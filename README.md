...
| [4/14] | SKIE: типы из Kotlin доходят до Swift без ручных обёрток | [смотреть](https://boosty.to/somestay/posts/81b3766e-e2af-4613-ba19-9c3cec6fdcac) | [v4](../../tree/v4) | [дифф](../../compare/v3...v4) |

Дальше по плану: DI на Koin, сеть на Ktor и так далее.

К [4/14] есть файл [steps/4-skie.md](steps/4-skie.md): все команды и вставки кода из видео по порядку, с таймкодами.
...
## Стек на момент текущей серии: 4/14
...
- **Room 2.8.4 и KSP 2.3.11** - база данных в общем коде, одна на iOS и Android. Код для запросов пишет KSP на сборке.
- **SKIE 0.10.14** - плагин компилятора от Touchlab. С ним Swift видит `StateFlow` с настоящим типом, зовёт `suspend` через `try await` с отменой и разбирает `sealed` через `switch`.
- **MVI** - актуальная для KMP архитектура, так же набирает популярность и в нативном iOS.
- **Watch** - чтобы понять внутрянку SKIE, в [2/14] и [3/14] мы писали этот хелпер руками. В [4/14] его заменил SKIE, файл удалён.
